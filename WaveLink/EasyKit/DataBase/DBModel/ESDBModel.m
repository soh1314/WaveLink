...
@implementation ESDBModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [ESDBModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    ESDBHelper *ESDB = [ESDBHelper shareInstance];
    [ESDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    ESDBHelper *ESDB = [ESDBHelper shareInstance];
    [ESDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[ESDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是JKBaseModel的子类
    for (ESDBModel *model in array) {
        if (![model isKindOfClass:[ESDBModel class]]) {
            return NO;
        }
    }
...
/** 批量保存用户对象 */
+ (BOOL)replaceObjects:(NSArray *)array
{
    //判断是否是JKBaseModel的子类
    for (ESDBModel *model in array) {
        if (![model isKindOfClass:[ESDBModel class]]) {
    ...
    [ESDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (ESDBModel *model in array) {
            ...
            NSString *sql = [NSString stringWithFormat:@"REPLACE INTO %@(%@) VALUES (%@);", tableName, keyString, valueString];
            BOOL flag = [db executeUpdate:sql withArgumentsInArray:insertValues];
            model.pk = flag?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
            NSLog(flag?@"插入成功":@"插入失败");
            if (!flag) {
...
+ (NSArray *)findAll
{
    ...
    [ESDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            ESDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
...
+ (NSString *)getColumeAndTypeString
{
    ...
    if ([self constranString]) {
        for (NSString *constrain in [self constranString]) {
            [pars appendString:constrain];
        }
    }
    return pars;
}
